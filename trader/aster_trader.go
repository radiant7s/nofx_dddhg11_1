...
func NewAsterTrader(user, signer, privateKeyHex string) (*AsterTrader, error) {
	...
	if err != nil {
		return nil, fmt.Errorf(name+"解析私钥失败: %w", err)
	}
...
func (t *AsterTrader) getPrecision(symbol string) (SymbolPrecision, error) {
	...
	return SymbolPrecision{}, fmt.Errorf(name+"未找到交易对 %s 的精度信息", symbol)
}
...
func (t *AsterTrader) sign(params map[string]interface{}, nonce uint64) error {
	...
	if err != nil {
		return fmt.Errorf(name+"ABI编码失败: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf(name+"签名失败: %w", err)
	}

	// 将v从0/1转换为27/28
	if len(sig) != 65 {
		return fmt.Errorf(name+"签名长度异常: %d", len(sig))
	}
	sig[64] += 27
...
func (t *AsterTrader) request(method, endpoint string, params map[string]interface{}) ([]byte, error) {
	...
	return nil, fmt.Errorf(name+"请求失败（已重试%d次）: %w", maxRetries, lastErr)
}
...
func (t *AsterTrader) doRequest(method, endpoint string, params map[string]interface{}) ([]byte, error) {
	...
	case "POST":
		...
		if resp.StatusCode != http.StatusOK {
			return nil, fmt.Errorf(name+"HTTP %d: %s", resp.StatusCode, string(body))
		}
		return body, nil
	...
	default:
		return nil, fmt.Errorf(name+"不支持的HTTP方法: %s", method)
	}
}
...
func (t *AsterTrader) GetBalance() (map[string]interface{}, error) {
	...
	if !foundUSDT {
		log.Printf(name + "⚠️  未找到USDT资产记录！")
	}

	// 获取持仓计算保证金占用和真实未实现盈亏
	positions, err := t.GetPositions()
	if err != nil {
		log.Printf(name+"⚠️  获取持仓信息失败: %v", err)
...
func (t *AsterTrader) OpenLong(symbol string, quantity float64, leverage int) (map[string]interface{}, error) {
	...
	if err := t.CancelAllOrders(symbol); err != nil {
		log.Printf(name+"  ⚠ 取消挂单失败(继续开仓): %v", err)
	}

	// 先设置杠杆
	if err := t.SetLeverage(symbol, leverage); err != nil {
		return nil, fmt.Errorf(name+"设置杠杆失败: %w", err)
	}
	...
	log.Printf(name+"  📏 精度处理: 价格 %.8f -> %s (精度=%d), 数量 %.8f -> %s (精度=%d)",
...
func (t *AsterTrader) CloseLong(symbol string, quantity float64) (map[string]interface{}, error) {
	...
	if quantity == 0 {
		...
		if quantity == 0 {
			return nil, fmt.Errorf(name+"没有找到 %s 的多仓", symbol)
		}
		log.Printf(name+"  📊 获取到多仓数量: %.8f", quantity)
	}
	...
	log.Printf(name+"✓ 平多仓成功: %s 数量: %s", symbol, qtyStr)

	// 平仓后取消该币种的所有挂单(止损止盈单)
	if err := t.CancelAllOrders(symbol); err != nil {
		log.Printf(name+"  ⚠ 取消挂单失败: %v", err)
	}
...
func (t *AsterTrader) CloseShort(symbol string, quantity float64) (map[string]interface{}, error) {
	...
	if quantity == 0 {
		...
		if quantity == 0 {
			return nil, fmt.Errorf(name+"没有找到 %s 的空仓", symbol)
		}
		log.Printf(name+"  📊 获取到空仓数量: %.8f", quantity)
	}
	...
	log.Printf(name+"✓ 平空仓成功: %s 数量: %s", symbol, qtyStr)
...
func (t *AsterTrader) SetMarginMode(symbol string, isCrossMargin bool) error {
	...
	if err != nil {
		...
		if strings.Contains(err.Error(), "No need to change") ||
			...
			log.Printf(name+"  ✓ %s 仓位模式已是 %s 或有持仓无法更改", symbol, marginType)
			return nil
		}
		// 检测多资产模式（错误码 -4168）
		if strings.Contains(err.Error(), "Multi-Assets mode") ||
			...
			strings.Contains(err.Error(), "4168") {
			log.Printf(name+"  ⚠️ %s 检测到多资产模式，强制使用全仓模式", symbol)
			log.Printf(name + "  💡 提示：如需使用逐仓模式，请在交易所关闭多资产模式")
			return nil
		}
		// 检测统一账户 API
		if strings.Contains(err.Error(), "unified") ||
			...
			strings.Contains(err.Error(), "Portfolio") {
			log.Printf(name+"  ❌ %s 检测到统一账户 API，无法进行合约交易", symbol)
			return fmt.Errorf(name + "请使用「现货与合约交易」API 权限，不要使用「统一账户 API」")
		}
		log.Printf(name+"  ⚠️ aster设置仓位模式失败: %v", err)
		// 不返回错误，让交易继续
		return nil
	}

	log.Printf(name+"  ✓ %s 仓位模式已设置为 %s", symbol, marginType)
	return nil
}
...
func (t *AsterTrader) GetMarketPrice(symbol string) (float64, error) {
	...
	body, _ := io.ReadAll(resp.Body)
	if resp.StatusCode != http.StatusOK {
		return 0, fmt.Errorf(name+"HTTP %d: %s", resp.StatusCode, string(body))
	}
...
func (t *AsterTrader) CancelStopLossOrders(symbol string) error {
	...
	if err != nil {
		return fmt.Errorf(name+"获取未完成订单失败: %w", err)
	}

	var orders []map[string]interface{}
	if err := json.Unmarshal(body, &orders); err != nil {
		return fmt.Errorf(name+"解析订单数据失败: %w", err)
	}
	...
	for _, order := range orders {
		...
		if orderType == "STOP_MARKET" || orderType == "STOP" {
			...
			if err != nil {
				...
				cancelErrors = append(cancelErrors, fmt.Errorf(name+"%s", errMsg))
				log.Printf(name+"  ⚠ 取消止损单失败: %s", errMsg)
				continue
			}

			canceledCount++
			log.Printf(name+"  ✓ 已取消止损单 (订单ID: %d, 类型: %s, 方向: %s)", int64(orderID), orderType, positionSide)
		}
	}

	if canceledCount == 0 && len(cancelErrors) == 0 {
		log.Printf(name+"  ℹ %s 没有止损单需要取消", symbol)
	} else if canceledCount > 0 {
		log.Printf(name+"  ✓ 已取消 %s 的 %d 个止损单", symbol, canceledCount)
	}

	// 如果所有取消都失败了，返回错误
	if len(cancelErrors) > 0 && canceledCount == 0 {
		return fmt.Errorf(name+"取消止损单失败: %v", cancelErrors)
	}
	...
	for _, order := range orders {
		...
		if orderType == "TAKE_PROFIT_MARKET" || orderType == "TAKE_PROFIT" {
			...
			if err != nil {
				...
				log.Printf(name+"  ⚠ 取消止盈单失败: %s", errMsg)
				continue
			}

			canceledCount++
			log.Printf(name+"  ✓ 已取消止盈单 (订单ID: %d, 类型: %s, 方向: %s)", int64(orderID), orderType, positionSide)
		}
	}

	if canceledCount == 0 && len(cancelErrors) == 0 {
		log.Printf(name+"  ℹ %s 没有止盈单需要取消", symbol)
	} else if canceledCount > 0 {
		log.Printf(name+"  ✓ 已取消 %s 的 %d 个止盈单", symbol, canceledCount)
	}

	// 如果所有取消都失败了，返回错误
	if len(cancelErrors) > 0 && canceledCount == 0 {
		return fmt.Errorf(name+"取消止盈单失败: %v", cancelErrors)
	}
...
func (t *AsterTrader) CancelStopOrders(symbol string) error {
	...
	for _, order := range orders {
		...
		if orderType == "STOP_MARKET" ||
			...
			if err != nil {
				log.Printf(name+"  ⚠ 取消订单 %d 失败: %v", int64(orderID), err)
				continue
			}

			canceledCount++
			log.Printf(name+"  ✓ 已取消 %s 的止盈/止损单 (订单ID: %d, 类型: %s)",
				symbol, int64(orderID), orderType)
		}
	}

	if canceledCount == 0 {
		log.Printf(name+"  ℹ %s 没有止盈/止损单需要取消", symbol)
	} else {
		log.Printf(name+"  ✓ 已取消 %s 的 %d 个止盈/止损单", symbol, canceledCount)
	}
